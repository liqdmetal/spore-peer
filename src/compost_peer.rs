...
//
//! compost-peer — the long-body P2P transport for Compost.
//!
...
//!
//! Usage:
//!   compost-peer serve --listen 0.0.0.0:8099 --dir <body-store-dir>   (sender)
//!   compost-peer fetch --addr host:8099 --cid <64-hex> [--out file]   (recipient)
//!
...
fn serve(addr: &str, dir: &str) -> std::io::Result<()> {
    ...
    eprintln!("compost-peer serve: listening on {addr}, bodies in {dir}");
...
fn main() {
    ...
    if args.len() < 2 {
        eprintln!("usage: compost-peer <serve|fetch> ...");
        std::process::exit(2);
    }
    match args[1].as_str() {
        "serve" => {
