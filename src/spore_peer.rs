...
struct ServeConfig {
    ...
    token: Option<String>,
    /// Relay-fabric state. None = fabric verbs refused (501 fabric disabled);
    /// Some = freg/fput/fpop served on this socket (the -fabric flag family).
    /// Arc-shared because ServeConfig clones per connection.
    fabric: Option<std::sync::Arc<FabricState>>,
}
...
fn handle_client(
    ...
    loop {
        ...
        // Fabric verbs: {"verb":"freg"|"fput"|"fpop", ...} — served only on
        // nodes that opted in (-fabric); others get an explicit "501 fabric
        // disabled" rather than falling into the cid path. A {"cid":...}
        // request never carries "verb", so the paths stay disjoint.
        if req.get("verb").and_then(|v| v.as_str()).is_some() {
            fabric::handle_verb(s, &req, dir, cfg, &ip);
            continue;
        }
        ...
        "serve" => {
            ...
            while i < args.len() {
                match args[i].as_str() {
                    ...
                    "-fabric" | "--fabric" => {
                        fabric_cfg.enabled = true;
                        i += 1;
                    }
                    "--fabric-per-handle" => {
                        if let Some(v) = args.get(i + 1).and_then(|s| s.parse().ok()) {
                            fabric_cfg.max_per_handle = v;
                        }
                        i += 2;
                    }
                    "--fabric-fput-rate" => {
                        if let Some(v) = args.get(i + 1).and_then(|s| s.parse().ok()) {
                            fabric_cfg.fput_rate = v;
                        }
                        i += 2;
                    }
                    "--fabric-max-regs" => {
            ...
            if fabric_cfg.enabled {
                // Rebuild the queue index from *.fenv before accepting
...
mod tests {
    ...
    pub(crate) fn mk_temp_dir(tag: &str) -> std::path::PathBuf {
